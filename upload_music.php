...
if(isset($_POST['submit'])){
    ...
    $album_folder ='./assets/uploaded_album/'.$album;
    ...
    $amusic= filter_var($music, FILTER_SANITIZE_STRING);
    $music_size = $_FILES['music']['size'];
    $music_tmp_name = $_FILES['music']['tmp_name'];
    $music_folder ='./assets/uploaded_music/'.$music;

    if($music_size > 100000000){
        $message[] = 'music size is too large!';
    }else{
        $insert_songs = $conn->prepare("INSERT INTO `songs` (name, artist, album, music)
         VALUES(?, ?, ?, ?)");
         $insert_songs->execute(array($name, $artist, $album, $music));
...
<section class="form-container">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="text" name="name" placeholder="enter music name" required
